...
st.subheader("Matrix:")
st.write(a2)
st.markdown(f"**First row:** {a2[0, :].tolist()}")
...
st.markdown(f"**Dot product:** {np.dot(a3, b3)}")
st.markdown(f"**Element-wise multiplication:** {a3 * b3}")
st.markdown("---")
...
st.markdown(f"**1D array:** {a4.tolist()}")
st.markdown("**Reshaped 3x4 matrix:**")
st.write(reshaped_a4)
st.markdown(f"**Flattened back to 1D array:** {flattened_a4.tolist()}")
st.markdown("---")

# Question 5
st.header("Question 5")
a5 = np.random.randint(1, 101, size=10)
st.markdown(f"**Random array:** {a5.tolist()}")
st.markdown(f"**Mean:** {np.mean(a5):.2f}")
st.markdown(f"**Median:** {np.median(a5)}")
st.markdown(f"**Standard deviation:** {np.std(a5):.2f}")
st.markdown("---")
...
st.markdown(f"**Original array:** {a6.tolist()}")
st.markdown(f"**Elements greater than 30:** {greater_than_30.tolist()}")
